...
static struct Mesh * LoadOBJ(const char *filename)
{
	...
	// Allocate arrays!
	if (vertCount > 0)
		theMesh->vertices = new GLfloat[vertCount];
	if (texCount > 0)
		theMesh->textureCoords = new GLfloat[texCount];
	if (normalsCount > 0)
		theMesh->vertexNormals = new GLfloat[normalsCount];
	if (hasPositionIndices)
		theMesh->coordIndex = new int[coordCount];
	if (hasNormalIndices)
		theMesh->normalsIndex = new int[coordCount];
	if (hasTexCoordIndices)
		theMesh->textureIndex = new int[coordCount];
...
void DecomposeToTriangles(struct Mesh *theMesh)
{
	...
	newCoords = new int[triangleCount * 3];
	if (theMesh->normalsIndex != NULL)
...
static void generateTangentsBi(Mesh* mesh)
{
	mesh->tangents = new GLfloat[3 * mesh->vertexCount];
	memset(mesh->tangents, 0, 3 * sizeof(GLfloat) * mesh->vertexCount);

	mesh->bitangents = new GLfloat[3 * mesh->vertexCount];
	memset(mesh->bitangents, 0, 3 * sizeof(GLfloat) * mesh->vertexCount);

	for (int i = 0; i < mesh->vertexCount; i++)
	{
		
		Point3D v0,v1,v2;
		SetVector(mesh->vertices[9*i], mesh->vertices[9 * i + 1], mesh->vertices[9 * i + 2], &v0);
		SetVector(mesh->vertices[9 * i+3], mesh->vertices[9 * i + 4], mesh->vertices[9 * i + 5], &v1);
		SetVector(mesh->vertices[9 * i + 6], mesh->vertices[9 * i + 7], mesh->vertices[9 * i + 8], &v2);
		
		Point3D uv0,uv1,uv2;
		SetVector(mesh->textureCoords[6*i], mesh->textureCoords[6 * i + 1], 0, &uv0);
		SetVector(mesh->textureCoords[6 * i+2], mesh->textureCoords[6 * i + 3], 0, &uv1);
		SetVector(mesh->textureCoords[6 * i + 4], mesh->textureCoords[6 * i + 5], 0, &uv2);

		Point3D deltaPos0, deltaPos1;

		VectorSub(&v1, &v0, &deltaPos0);
		VectorSub(&v2, &v0, &deltaPos1);

		Point3D deltaUV0, deltaUV1;

		VectorSub(&uv1, &uv0, &deltaUV0);
		VectorSub(&uv2, &uv0, &deltaUV1);

		Point3D tangent, bitangent ,temp0, temp1;

		float r = 1.0f / (deltaUV0.x * deltaUV1.y - deltaUV0.y * deltaUV1.x);

		ScalarMult(&deltaPos0, r*deltaUV1.y, &temp0);
		ScalarMult(&deltaPos1, r*deltaUV0.y, &temp1);
		VectorSub(&temp0, &temp1, &tangent);

		ScalarMult(&deltaPos1, r*deltaUV0.x, &temp0);
		ScalarMult(&deltaPos0, r*deltaUV1.x, &temp1);
		VectorSub(&temp0, &temp1, &bitangent);

		mesh->tangents[3*i] = tangent.x;
		mesh->tangents[3 * i+1] = tangent.y;
		mesh->tangents[3 * i+2] = tangent.z;

		mesh->bitangents[3 * i] = bitangent.x;
		mesh->bitangents[3 * i + 1] = bitangent.y;
		mesh->bitangents[3 * i + 2] = bitangent.z;
	}
}
...
static Model* generateModel(Mesh* mesh)
{
	...
	IndexTriplet* indexHashMap = new IndexTriplet[indexHashMapSize];
	...
	model->indexArray = new GLuint[mesh->coordCount];
	...
	if (mesh->vertices)
		model->vertexArray = new GLfloat[3 * numNewVertices];
	if (mesh->vertexNormals)
		model->normalArray = new GLfloat[3 * numNewVertices];
	if (mesh->textureCoords)
		model->texCoordArray = new GLfloat[2 * numNewVertices];

	model->tangentArray	= new GLfloat[3 * numNewVertices];
	model->bitangentArray = new GLfloat[3 * numNewVertices];
	...
	for (index = 0; index < indexHashMapSize; index++)
	{
		if (indexHashMap[index].newIndex != -1)
		{
			...
			if (mesh->tangents)
				memcpy(&model->tangentArray[3 * indexHashMap[index].newIndex],
					&mesh->tangents[3 * indexHashMap[index].normalIndex],
					3 * sizeof(GLfloat));

			if (mesh->bitangents)
				memcpy(&model->bitangentArray[3 * indexHashMap[index].newIndex],
					&mesh->bitangents[3 * indexHashMap[index].normalIndex],
					3 * sizeof(GLfloat));
		}
	}
